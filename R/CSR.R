...
CSR_assign <- function(dAtA, var.name, p_adj, p.value.cutoff, Parallel, Vis) {
  if(missing(dAtA)) print("No data input!") else {
    ...
    for (i in unique(filtered_dAtA[,1])){
      ...
      if (n.Exp.Grp > 2) {
        if(all(dAtA_subset[1:(n.Exp.Grp-1),11] != "ns") & (all(dAtA_subset[1:(n.Exp.Grp-1),12] == "medium") | all(dAtA_subset[1:(n.Exp.Grp-1),12] == "large")) & any(dAtA_subset[1:(n.Exp.Grp-1),4] > 0)) {
          a[i,2] <- "C"} else if (all(dAtA_subset[(n.Exp.Grp-1):(2*n.Exp.Grp-3),11] != "ns") & (all(dAtA_subset[(n.Exp.Grp-1):(2*n.Exp.Grp-3),12] == "medium") | all(dAtA_subset[(n.Exp.Grp-1):(2*n.Exp.Grp-3),12] == "large")) & any(dAtA_subset[(n.Exp.Grp-1):(2*n.Exp.Grp-3),4] < 0)) {
