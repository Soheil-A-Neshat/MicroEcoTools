test_that("CSR_Simulation runs and returns expected output", {
  ...
  sim_results <- CSR_Simulation(DaTa = CSR_IP2G_data, NSim = 1, p_adj = "BH", var.name = "Trait", NuLl.test = FALSE, Keep_data = FALSE, Parallel = TRUE)
  # Check that the final verdict has the proper class
  expect_true(inherits(sim_results$Final_Verdict, "Final_verdict"))
